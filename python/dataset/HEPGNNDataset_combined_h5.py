...
class HEPGNNDataset_combined_h5(PyGDataset):
    def __init__(self, **kwargs):
        super(HEPGNNDataset_combined_h5, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
...
#             self.sampleInfo = pd.concat([self.sampleInfo, info], ignore_index=True)
    ...
    def initialize(self,cla_type,fea_num):
        ...
        for i, fName in enumerate(self.sampleInfo['fileName']):
            ...
            for j in range(nEvent):
                ...
                f_fea_list.append(f_fea_reshape[:,:fea_num])
