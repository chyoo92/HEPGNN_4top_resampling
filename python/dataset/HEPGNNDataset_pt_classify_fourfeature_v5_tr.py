...
class HEPGNNDataset_pt_classify_fourfeature_v5_tr(PyGDataset):
    def __init__(self, **kwargs):
        super(HEPGNNDataset_pt_classify_fourfeature_v5_tr, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
    ...
    def __getitem__(self, idx):
        ...
        data = copy.deepcopy(self.graphList[fileIdx][idx])

        
        label = copy.deepcopy(self.labelList[fileIdx][idx])
        weight = copy.deepcopy(self.weightList[fileIdx][idx])
        real_weight = copy.deepcopy(self.real_weightList[fileIdx][idx])
        btag = copy.deepcopy(self.btag_List[fileIdx][idx])
        rescale = copy.deepcopy(self.rescaleList[fileIdx][idx])
        procIdxs = copy.deepcopy(self.procList[fileIdx][idx])
        
        eval_resam = copy.deepcopy(self.eval_resamwList[fileIdx][idx])
        eval_real = copy.deepcopy(self.eval_realwList[fileIdx][idx])
